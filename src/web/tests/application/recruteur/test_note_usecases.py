...
def test_creer_note_persists_and_drains_events(repository) -> None:
...
def test_editer_note_updates_message(repository) -> None:
    audit_log_writer = MagicMock(spec=AuditLogWriter)
    candidature_id = uuid4()
    auteur_id = uuid4()
...
def test_editer_note_raises_when_not_publisher(repository) -> None:
    candidature_id = uuid4()
    existing = Note.create(
...
def test_editer_note_raises_when_candidature_mismatch(repository) -> None:
    existing = Note.create(
...
def test_supprimer_note_marks_deleted_and_drains(repository) -> None:
    audit_log_writer = MagicMock(spec=AuditLogWriter)
    candidature_id = uuid4()
    auteur_id = uuid4()
    existing = Note.create(
...
def test_supprimer_note_raises_when_not_publisher(repository) -> None:
    candidature_id = uuid4()
    existing = Note.create(
...
def test_lister_notes_returns_repository_result(repository) -> None:
    candidature_id = uuid4()
    note = Note.create(
        candidature_id=candidature_id, publie_par_id=uuid4(), message="note"
    )
