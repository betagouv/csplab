...
class TestPurgeApiLogsTask:
    def test_deletes_logs_older_than_retention(self, db):
        ApiLogDjangoFactory(timestamp=date_to_aware_datetime(OLD_DATE))
    ...
    def test_keeps_logs_within_retention(self, db):
        ApiLogDjangoFactory(timestamp=date_to_aware_datetime(RECENT_DATE))
    ...
    def test_keeps_logs_on_cutoff_date(self, db):
        ApiLogDjangoFactory(timestamp=date_to_aware_datetime(CUTOFF_DATE))
