...
def test_create_note(candidature_id, agent_id) -> None:
...
def test_save_persists_message_edit(candidature_id, agent_id) -> None:
    ...
    note.modifier(message=nouveau_message, mis_a_jour_par_id=agent_id)
...
def test_list_by_candidature_returns_only_its_notes_ordered(
    ...
    note_old = NoteFactory.create_model(
        candidature_id=candidature_id, publie_par_id=agent_id
    )
    ...
    assert [n.entity_id for n in notes] == [note_recent.id, note_old.id]
