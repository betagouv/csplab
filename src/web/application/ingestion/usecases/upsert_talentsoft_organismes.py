...
class UpsertTalentsoftOrganismesUsecase:
    def execute(self, items: list[dict]) -> dict:
        ...
        duplicate_entity_codes = {
            entity_code
            for entity_code, count in entity_code_counts.items()
            if count > 1
        }
        ...
        existing_entity_codes = set(
            TalentsoftOrganismeModel.objects.filter(
                entity_code__in=[item["entity_code"] for item in valid_items]
