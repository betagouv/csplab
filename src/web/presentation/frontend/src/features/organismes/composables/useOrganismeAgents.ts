import type { ModifierAgentPayload } from '../types'
import { useMutation, useQuery, useQueryCache } from '@pinia/colada'
import { updateOrganismeAgent } from '../api'
...
export function useOrganismeAgents(organismeUuid: string) {
  ...
  const updateMutation = useMutation({
    mutation: (payload: ModifierAgentPayload) => updateOrganismeAgent(organismeUuid, payload),
