...
import { getOrganismeAgents } from '@/features/organismes/api'
import { MEMBRE_EQUIPE, RECRUTEMENT_UUID } from '@/test/fixtures/equipe'
import { AGENT_ORGANISME } from '@/test/fixtures/organismes'
...
const NOUVEL_AGENT = { ...AGENT_ORGANISME, agent_id: 'bbbbbbbb-0002-0002-0002-000000000002', email: 'paul.bernard@example.gouv.fr' }
...
describe('equipeRecrutementSection', () => {
  beforeEach(() => {
    ...
    vi.mocked(getOrganismeAgents).mockReset().mockResolvedValue([AGENT_ORGANISME, NOUVEL_AGENT])
    vi.mocked(addMembreEquipe).mockReset()
    vi.mocked(updateMembreEquipe).mockReset()
  })

  it.each([
    [409, 'Cet agent fait déjà partie de l\'équipe'],
    [404, 'Cet agent n\'est plus rattaché à l\'organisme'],
    ...
    await user.click(await screen.findByRole('combobox', { name: 'Membre de l\'organisme' }))
    await user.click(await screen.findByRole('option', { name: NOUVEL_AGENT.email }))
