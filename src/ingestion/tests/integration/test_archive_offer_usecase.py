...
async def test_execute_posts_to_archive_endpoint(
    archive_offer_usecase: ArchiveOfferUsecase, httpx_mock: HTTPXMock
):
    httpx_mock.add_response(method="POST", url=ARCHIVE_URL, status_code=200)
    await archive_offer_usecase.execute(REFERENCE, source_id=SOURCE_ID)
...
async def test_execute_marks_offer_as_archived_in_repository(
    archive_offer_usecase: ArchiveOfferUsecase,
