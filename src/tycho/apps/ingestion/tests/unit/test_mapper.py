...
from apps.ingestion.infrastructure.adapters.external.talentsoft.offersummaries_mapper import OfferSummariesMapper

def main() -> None:
    # tu crées ce fichier en copiant une vraie réponse JSON de l'API
    raw = json.loads(Path("scripts/talentsoft/offersummaries_sample.json").read_text(encoding="utf-8"))
