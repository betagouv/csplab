...
from infrastructure.django_apps.ingestion.management.commands.load_documents import (
...
class TestLoadDocumentsCommand:
    ...
    def test_operation_type_mapping_completeness(self):
        """Test that all document types have operation type mappings."""
        assert (
            OPERATION_TYPE_MAPPING[DocumentType.OFFERS]
            == LoadOperationType.FETCH_FROM_TALENTSOFT_FRONT_API
        )
        assert (
            OPERATION_TYPE_MAPPING[DocumentType.CORPS]
            == LoadOperationType.FETCH_FROM_INGRES_API
        )

    def test_command_raises_error_for_unsupported_document_type(self):
        """Test that command raises CommandError for unmatched document types."""
        with patch(
            "infrastructure.django_apps.ingestion.management.commands.load_documents.OPERATION_TYPE_MAPPING",
            {},
        ):
            with pytest.raises(CommandError, match="Unsupported document type: OFFERS"):
                call_command("load_documents", "--type", DocumentType.OFFERS.value)

    @pytest.mark.parametrize(
        "document_type,expected_operation",
        [
            (
                DocumentType.OFFERS.value,
                LoadOperationType.FETCH_FROM_TALENTSOFT_FRONT_API,
            ),
            (DocumentType.CORPS.value, LoadOperationType.FETCH_FROM_INGRES_API),
        ],
    )
    def test_command_calls_usecase_with_correct_parameters(
        ...
        assert call_args.operation_type == expected_operation
