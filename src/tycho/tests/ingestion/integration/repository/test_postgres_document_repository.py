...
class TestFindByType:
    ...
    def test_offsetting(self, db, repository, start, fetched_docs, expected_has_more):
        ...
        RawDocumentFactory.create_model_batch(total_docs, document_type)
    ...
    def test_filtering_with_mixed_document_type(self, db, repository):
        ...
        for document_type in DocumentType:
            RawDocumentFactory.create_model_batch(
    ...
    def test_fetch_by_type_returns_documents_in_id_order(self, db, repository):
        ...
        RawDocumentFactory.create_model(
            document_type=document_type, external_id="uuid-1"
        )
        RawDocumentFactory.create_model(
            document_type=document_type, external_id="uuid-2"
        )
...
class TestFindByExternalIds:
    ...
    def test_ignore_other_document_type(self, db, repository):
        external_id = "uuid-1"
        raw_document = RawDocumentFactory.create_model(
...
class TestUpsertBatch:
    def test_datetime_on_upsert(self, db, repository):
        raw_doc = RawDocumentFactory.create_model()
        raw_doc_to_update = RawDocumentFactory.create_model()
        new_raw_doc = RawDocumentFactory.create_model(save_in_db=False)
...
class TestGetPendingProcessing:
    def test_excluded_items(self, db, repository):
        RawDocumentFactory.create_model(document_type=DocumentType.CORPS)
        RawDocumentFactory.create_model(document_type=DocumentType.CONCOURS)
        RawDocumentFactory.create_model(processing=True)
        RawDocumentFactory.create_model(processed_at=NOW, updated_at=DAY_AGO)
    ...
    def test_get_pending_items_with_logical_lock(self, db, repository):
        never_processed = RawDocumentFactory.create_model()
        updated_after_processed = RawDocumentFactory.create_model(
    ...
    def test_limit(self, db, repository):
        RawDocumentFactory.create_model_batch(2)
...
def test_mark_as_processed(db, repository):
    raw_documents = [
        RawDocumentFactory.create_model(processing=True).to_entity(),
        RawDocumentFactory.create_model(processing=False).to_entity(),
    ]
    undesired_raw_document = RawDocumentFactory.create_model(
        processing=True
    ).to_entity()
