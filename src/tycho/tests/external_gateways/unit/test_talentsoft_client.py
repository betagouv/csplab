...
class TestGetOffers:
    ...
    async def test_get_offers(self, talentsoft_client, expected_has_more):
        ...
        # Setup
        expected_offers = [
            {"id": fake.uuid4(), "title": fake.job()},
            {"id": fake.uuid4(), "title": fake.job()},
        ]
        mock_response = mocked_response(
            return_value={
                "data": expected_offers,
                "_pagination": {"hasMore": expected_has_more, "total": 2},
            }
        )
    ...
    async def test_get_offers_handles_empty_data(self, talentsoft_client):
        """Test get_offers handles response with empty or missing data."""
        # Setup - response without data key
    ...
    async def test_get_offers_fails_due_to_unauthorized_token(self, talentsoft_client):
        ...
        expected_offers = [{"id": fake.uuid4(), "title": fake.job()}]
        ...
        success_response = mocked_response(
            return_value={"data": expected_offers, "_pagination": {"hasMore": False}}
        )
