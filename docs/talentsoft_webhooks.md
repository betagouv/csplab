...
# Get an OAuth token
TOKEN=$(curl -s -d "client_id=${WEB_TALENTSOFT_BACK_CLIENT_ID}&client_secret=${WEB_TALENTSOFT_BACK_CLIENT_SECRET}&grant_type=client_credentials&scope=Customer" -X POST "${WEB_TALENTSOFT_BACK_BASE_URL}api/token" | jq -r '.access_token')
...
for event in "${EVENTS[@]}"; do
    ...
    curl -s -o /dev/null -w "%{http_code}" \
      ...
      -X POST "${WEB_TALENTSOFT_BACK_BASE_URL}api/system/v1/webhooks"
    echo ""
done
```
