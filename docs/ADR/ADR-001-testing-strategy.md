...
- Python factories are reused across tests to produce coherent, consistent datasets
...
- Mocking external gateways means integration failures with third parties can go
  undetected until staging or production
...
- [ ] Remove all in-memory repository tests — replace with pytest tests against real
      repository methods covering edge cases
