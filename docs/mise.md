...
Les workflows `web.yml`, `ingestion.yml` et `ocr.yml` sélectionnent l'environnement `MISE_ENV=ci`, qui charge `mise.ci.toml` par-dessus `mise.toml`, qui remplace les tâches `services:*` par des no-ops, les services étant fournis par les containers du workflow. `src/ingestion/mise.ci.toml` neutralise en plus ses tâches `db:create*` et pointe `DATABASE_URL`/`TEST_DATABASE_URL` vers le service Postgres du job. `src/ocr/mise.ci.toml` fournit l'`API_KEY` que `env.d/ocr` (non versionné) porte en local.
